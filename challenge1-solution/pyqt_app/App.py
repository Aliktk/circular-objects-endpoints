...
class CoinDetectorApp(QMainWindow):
    def __init__(self):
        ...
        self.init_ui()
        self.load_yolo_models()  # Load YOLOv8 models

    def load_yolo_models(self):
        """Load YOLOv8 Detection and Segmentation models."""
        try:
            # Initialize YOLOv8 Detection Model
            self.yolo_detection_model = YOLO('D:/All Projects/Interview Assignments/Brightminds/Challenge 1 solution/yolo-training/best.pt')  # Replace with your detection model path
            logging.info('YOLOv8 Detection model loaded successfully.')

            # Initialize YOLOv8 Segmentation Model
            self.yolo_segmentation_model = YOLO('D:/All Projects/Interview Assignments/Brightminds/Challenge 1 solution/yolo_segmentation/best.pt')  # Replace with your segmentation model path
            logging.info('YOLOv8 Segmentation model loaded successfully.')
        except Exception as e:
            logging.error(f'Error loading YOLOv8 models: {e}')
            QMessageBox.critical(self, 'Error', f'Failed to load YOLOv8 models:\n{e}')
            self.yolo_detection_model = None
            self.yolo_segmentation_model = None
    ...
    def init_ui(self):
        ...
        methods = ['Contour Detection', 'Watershed Algorithm', 'YOLOv8 Detection', 'YOLOv8 Segmentation']
        ...
        self.create_yolo_detection_parameters()
        self.create_yolo_segmentation_parameters()
        ...
        overlay_label = QLabel('Processed Image with Overlay')
    ...
    def create_yolo_detection_parameters(self):
        """Create parameter widget for YOLOv8 Detection."""
        try:
            widget = QWidget()
            layout = QFormLayout()
            ...
            # Store references to sliders and labels
            self.parameter_controls['YOLOv8 Detection'] = {
                'confidence': (conf_slider, conf_label),
                'nms': (nms_slider, nms_label)
            }
        except Exception as e:
            logging.error(f'Error creating YOLOv8 Detection parameters: {e}')
            QMessageBox.critical(self, 'Error', f'An error occurred while creating YOLOv8 Detection parameters:\n{e}')

    def create_yolo_segmentation_parameters(self):
        ...
        try:
            ...
            logging.error(f'Error creating YOLOv8 Segmentation parameters: {e}')
            QMessageBox.critical(self, 'Error', f'An error occurred while creating YOLOv8 Segmentation parameters:\n{e}')
    ...
    def process_image(self):
        try:
            ...
            elif self.method == 'YOLOv8 Detection':
                self.process_with_yolo_detection()
            elif self.method == 'YOLOv8 Segmentation':
                self.process_with_yolo_segmentation()
    ...
    def process_with_yolo_detection(self):
        try:
            if self.yolo_detection_model is None:
                logging.error('YOLOv8 Detection model is not loaded.')
                QMessageBox.critical(self, 'Error', 'YOLOv8 Detection model is not loaded.')
                return

            # Retrieve YOLOv8 Detection parameters from parameter_controls
            conf_slider, conf_label = self.parameter_controls['YOLOv8 Detection']['confidence']
            nms_slider, nms_label = self.parameter_controls['YOLOv8 Detection']['nms']

            confidence_threshold = conf_slider.value() / 100.0
            nms_threshold = nms_slider.value() / 100.0

            logging.info(f'YOLOv8 Detection Parameters - Confidence Threshold: {confidence_threshold}, NMS Threshold: {nms_threshold}')

            # Perform inference
            results = self.yolo_detection_model.predict(self.image, conf=confidence_threshold, iou=nms_threshold)

            # Create copies for overlay and segmentation display
            overlay_img = self.image.copy()
            semantic_img = np.zeros_like(self.image)
            ...
            for result in results:
                for box in result.boxes.data.tolist():
                    ...
                    class_name = self.yolo_detection_model.names[int(cls)]
                    if class_name.lower() != 'coin':
                        continue  # Skip non-coin detections

                    # Draw bounding box
                    cv2.rectangle(overlay_img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                    cv2.putText(overlay_img, f'Coin: {conf:.2f}', (int(x1), int(y1)-10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)

                    # Extract centroid
                    centroid_x = int((x1 + x2) / 2)
                    centroid_y = int((y1 + y2) / 2)
                    cv2.circle(overlay_img, (centroid_x, centroid_y), 2, (0, 0, 255), 3)

                    # Create mask for segmentation display (optional)
                    mask = np.zeros_like(self.image)
                    cv2.rectangle(mask, (int(x1), int(y1)), (int(x2), int(y2)), (255, 255, 255), -1)
                    segmented = cv2.bitwise_and(self.image, self.image, mask=cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY))
                    semantic_img[mask[:, :, 0] == 255] = segmented[mask[:, :, 0] == 255]

                    count += 1
                    logging.info(f'Detected YOLOv8 coin #{count}: Confidence={conf:.2f}, Centroid=({centroid_x}, {centroid_y})')

            # Overlay the count on the image
            cv2.putText(overlay_img, f'Count: {count}', (10, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 3, cv2.LINE_AA)
            logging.info(f'Number of YOLOv8 detected coins: {count}')

            self.processed_image = overlay_img
            self.display_segmented_coins(semantic_img, count)
        except Exception as e:
            logging.error(f'Error in YOLOv8 Detection: {e}')
            QMessageBox.critical(self, 'Error', f'An error occurred during YOLOv8 Detection:\n{e}')

    def process_with_yolo_segmentation(self):
        try:
            if self.yolo_segmentation_model is None:
                logging.error('YOLOv8 Segmentation model is not loaded.')
                QMessageBox.critical(self, 'Error', 'YOLOv8 Segmentation model is not loaded.')
                return

            # Retrieve YOLOv8 Segmentation parameters from parameter_controls
            ...
            logging.info(f'YOLOv8 Segmentation Parameters - Confidence Threshold: {confidence_threshold}, NMS Threshold: {nms_threshold}')

            # Perform segmentation inference
            results = self.yolo_segmentation_model.predict(self.image, conf=confidence_threshold, iou=nms_threshold)
            ...
            for result in results:
                for mask, box, conf, cls in zip(result.masks.data, result.boxes.xyxy, result.boxes.conf, result.boxes.cls):
                    class_name = self.yolo_segmentation_model.names[int(cls)]
                    if class_name.lower() != 'coin':
                        continue  # Skip non-coin detections
                    ...
                    segmented = cv2.bitwise_and(self.image, self.image, mask=mask_binary)
                    semantic_img[mask_binary == 255] = segmented[mask_binary == 255]

                    count += 1
                    logging.info(f'Detected YOLOv8 Segmented coin #{count}: Confidence={conf:.2f}, Centroid=({centroid_x}, {centroid_y})')

            # Overlay the count on the image
            cv2.putText(overlay_img, f'Count: {count}', (10, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 3, cv2.LINE_AA)
            logging.info(f'Number of YOLOv8 segmented coins: {count}')
